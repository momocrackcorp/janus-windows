...
$themeRoot = Join-Path $repo "assets\windows-themes\papirus"
...
$navy = [System.Drawing.Color]::FromArgb(72, 42, 27)
$blue = [System.Drawing.Color]::FromArgb(166, 83, 42)
$gold = [System.Drawing.Color]::FromArgb(38, 166, 91)
$cream = [System.Drawing.Color]::FromArgb(240, 224, 190)
$outline = [System.Drawing.Color]::FromArgb(38, 28, 20)
...
function Add-ArrowShape([System.Drawing.Graphics]$graphics, [int]$offsetX = 0, [int]$offsetY = 0, [double]$scale = 1.0) {
    ...
    $points = [System.Drawing.PointF[]]@(
        [System.Drawing.PointF]::new([float](7*$scale+$offsetX),[float](4*$scale+$offsetY)),
        [System.Drawing.PointF]::new([float](44*$scale+$offsetX),[float](35*$scale+$offsetY)),
        [System.Drawing.PointF]::new([float](29*$scale+$offsetX),[float](37*$scale+$offsetY)),
        [System.Drawing.PointF]::new([float](38*$scale+$offsetX),[float](54*$scale+$offsetY)),
        [System.Drawing.PointF]::new([float](27*$scale+$offsetX),[float](60*$scale+$offsetY)),
        [System.Drawing.PointF]::new([float](18*$scale+$offsetX),[float](42*$scale+$offsetY)),
        [System.Drawing.PointF]::new([float](8*$scale+$offsetX),[float](51*$scale+$offsetY))
    )
    ...
    $graphics.DrawLine($goldPen,[float](11*$scale+$offsetX),[float](12*$scale+$offsetY),[float](22*$scale+$offsetX),[float](40*$scale+$offsetY))
...
function New-ToneWav([string]$name, [double[]]$frequencies, [int]$millisecondsPerTone) {
    ...
    foreach ($frequency in $frequencies) {
        ...
        for ($index = 0; $index -lt $count; $index++) {
            ...
            $progress = $index / [Math]::Max(1, $count)
            $attack = [Math]::Min(1.0, $index / ($sampleRate * 0.012))
            $release = [Math]::Max(0, ($count - $index - 1) / ($sampleRate * 0.045))
            $envelope = $attack * [Math]::Min(1.0, $release) * [Math]::Exp(-2.8 * $progress)
            $reed = [Math]::Sin(2 * [Math]::PI * $frequency * $time) + 0.30 * [Math]::Sin(2 * [Math]::PI * ($frequency * 2.00) * $time) + 0.12 * [Math]::Sin(2 * [Math]::PI * ($frequency * 3.00) * $time)
            $oud = (0.48 * [Math]::Sin(2 * [Math]::PI * ($frequency * 1.005) * $time) + 0.22 * [Math]::Sin(2 * [Math]::PI * ($frequency * 2.01) * $time)) * [Math]::Exp(-6.5 * $progress)
            $breath = 0.055 * [Math]::Sin(2 * [Math]::PI * ($frequency * 5.37) * $time) * [Math]::Sin(2 * [Math]::PI * 17 * $time)
            $frameDrum = if ($toneIndex -eq 0) { 0.18 * [Math]::Sin(2 * [Math]::PI * (92 - 34 * $progress) * $time) * [Math]::Exp(-18 * $progress) } else { 0 }
            $wave = 0.62 * $reed + 0.34 * $oud + $breath + $frameDrum
            $samples.Add([Int16]([Math]::Max(-32767, [Math]::Min(32767, 4300 * $wave * $envelope))))
        }
        1..441 | ForEach-Object { $samples.Add([Int16]0) }
        $toneIndex++
    }
...
function New-WallpaperVariants([string]$sourceName, [string]$variant, [ValidateSet("Top", "Center", "Bottom")][string]$ultrawideAlignment) {
    ...
    Save-Jpeg $standard (Join-Path $wallpaperDir ("JANUS-Papirus-" + $variant + "-4K.jpg"))
    ...
    Save-Jpeg $ultrawide (Join-Path $wallpaperDir ("JANUS-Papirus-" + $variant + "-Ultrawide-5K.jpg"))
    $wideGraphics.Dispose(); $ultrawide.Dispose(); $source.Dispose()
}

New-Cursor "papirus-arrow.cur" "arrow" 7 5
New-Cursor "papirus-help.cur" "help" 7 5
New-Cursor "papirus-hand.cur" "hand" 30 17
New-Cursor "papirus-ibeam.cur" "ibeam" 32 32
New-Cursor "papirus-cross.cur" "cross" 32 32
New-Cursor "papirus-move.cur" "move" 32 32
New-Cursor "papirus-size-ns.cur" "ns" 32 32
New-Cursor "papirus-size-we.cur" "we" 32 32
New-Cursor "papirus-size-nwse.cur" "nwse" 32 32
New-Cursor "papirus-size-nesw.cur" "nesw" 32 32
New-Cursor "papirus-up.cur" "up" 32 3
New-Cursor "papirus-no.cur" "no" 7 5
New-Cursor "papirus-pen.cur" "pen" 9 56
New-AnimatedCursor "papirus-working.ani" $true
New-AnimatedCursor "papirus-busy.ani" $false

New-ToneWav "papirus-notify.wav" @(587.33, 622.25, 739.99, 783.99) 78
New-ToneWav "papirus-question.wav" @(587.33, 739.99, 622.25) 105
New-ToneWav "papirus-warning.wav" @(392.00, 369.99, 311.13, 293.66) 95
New-ToneWav "papirus-error.wav" @(293.66, 311.13, 293.66) 130
New-ToneWav "papirus-complete.wav" @(293.66, 311.13, 369.99, 392.00) 85
New-ToneWav "papirus-start.wav" @(293.66, 311.13, 369.99, 392.00, 440.00, 392.00) 95
New-ToneWav "papirus-logon.wav" @(293.66, 369.99, 392.00) 110
New-ToneWav "papirus-logoff.wav" @(392.00, 369.99, 311.13, 293.66) 105
New-ToneWav "papirus-exit.wav" @(440.00, 392.00, 369.99, 311.13, 293.66) 100

$baseIcons = Join-Path $repo "assets\theme-packs\papirus"
...
Set-Content -LiteralPath (Join-Path $iconDir "TEMA.txt") -Value "papirus" -Encoding UTF8

New-WallpaperVariants "JANUS-Papirus-Delta-source.png" "Delta" "Top"
New-WallpaperVariants "JANUS-Papirus-Piramides-source.png" "Piramides" "Top"
New-WallpaperVariants "JANUS-Papirus-Jeroglificos-source.png" "Jeroglificos" "Center"
New-Preview "JANUS-Papirus-Delta-4K.jpg" "JANUS-Papirus-Delta-preview.png"
New-Preview "JANUS-Papirus-Piramides-4K.jpg" "JANUS-Papirus-Piramides-preview.png"
New-Preview "JANUS-Papirus-Jeroglificos-4K.jpg" "JANUS-Papirus-Jeroglificos-preview.png"

$manifest = [ordered]@{
    id = "janus-papirus"
    version = 1
    displayName = "JANUS Papirus"
    publisher = "Momocrackcorp"
    variants = @("Delta", "Piramides", "Jeroglificos")
    wallpapers = @("Delta", "Piramides", "Jeroglificos")
    slideshowMinutes = 30
    preserveWindowsMode = $true
    components = @("wallpapers", "colors", "cursors", "sounds")
    accentColor = "#26A65B"
    secondaryAccentColor = "#A6532A"
    soundStyle = "Original Egyptian-inspired reed, plucked string and frame drum timbres"
    iconCompanion = "Tema-Iconos-Papirus.zip"
    safe = $true
    reversible = $true
}
$manifest | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath (Join-Path $themeRoot "manifest.json") -Encoding UTF8

$themeZip = Join-Path $dist "JANUS-Papirus-v1.zip"
$iconsZip = Join-Path $dist "JANUS-Papirus-Iconos-v1.zip"
